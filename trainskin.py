...
from data.dataloadskin import BasicDataset
# from utils.DCAN_CVV import ACM
from utils.CVV import ACM
...
def get_args():
    ...
    parser.add_argument('--lr-step', type=int, default=250, help='LR scheduler step')

    # Architecture
    parser.add_argument('--arch', type=str, default='unext',
                        choices=['mynet', 'denseUnet'],
                        help='Network architecture. unet or resnet')
    parser.add_argument('--ablation', type=str, default='BCE + ',
                        choices=['activeLoss', 'bd_loss', 'BCEDLoss'],
                        help='Network architecture.')

    # ACM
    parser.add_argument('--acm_iter', type=int, default=5, help='ACM iter.')

    # Data
    parser.add_argument('--train-dataset', type=str, default='skin', help='Training dataset')
    parser.add_argument('--data-path', type=str, default='/root/projects/Datasets/skin',
...
def train_epoch(model, criterion1, criterion2, optimizer, data_loader, epoch, device, summary):
    model.train()
    ...
    for idx, image_and_mask in enumerate(iterator):
        ...
        cnn_pred = model(image)
        mask_pred = ACM(cnn_pred, image, mask, args.acm_iter)
        # flops, params = profile(model, image)
        # print('flops: %.2f M, params: %.2f M' % (flops / 1000000.0, params / 1000000.0))

        # 获取loss
        loss1 = criterion1(cnn_pred, mask)
        loss2 = criterion2(mask_pred, mask)
        ...
        summary.visualize_image('Train', image, mask, cnn_pred, mask_pred, global_step)
...
def val_epoch(model, data_loader, epoch, device, summary):
    ...
    for idx, image_and_mask in enumerate(data_loader):
        ...
        # mask_pred = ACM(cnn_pred, image, mask_gt, args.acm_iter)

        mask = mask_gt

        dice = metrics.dice_coeff(mask_pred, mask)
        valuate_dice.update(dice)
        valuate_iou.update(metrics.iou_score(mask_pred, mask))
        precision, recall, accuracy = metrics.BoundF(mask_pred, mask)
        hausdorff = 0

        for pred_hd, mask_gt in zip(mask_pred, mask_gt):
            pred_hd = (pred_hd > 0.5).float()  # mask_pred.size(1,480,480)
            haus = metrics.Hausdorf(pred_hd, mask_gt)
            hausdorff = haus
            valuate_huass.update(metrics.Hausdorf(pred_hd, mask_gt), pred_hd.size(0))
        ...
        summary.visualize_image('Val', image, mask, mask_pred, mask_pred, global_step)
...
if __name__ == '__main__':
    ...
    train_img_path = glob(args.data_path + '/train/imgs/*')
    train_label_path = glob(args.data_path + '/train/masks/*')
    test_img_path = glob(args.data_path + '/test/imgs/*')
    test_label_path = glob(args.data_path + '/test/masks/*')
    train_img_path.sort()
    train_label_path.sort()
    test_img_path.sort()
    test_label_path.sort()

    train_data = BasicDataset(train_img_path, train_label_path, train=True, transform=True)
    val_data = BasicDataset(test_img_path, test_label_path, train=False)
    ...
    criterion1 = loss_function.BCEDiceLoss()
    criterion2 = myloss.MyLoss()
    ...
    best_dice = 0
    log = pd.DataFrame(index=[], columns=['epoch', 'val_dice', 'val_hausdorff', 'val_iou', 'precision', 'recall', 'accuracy'])
    for epoch in range(args.start_epoch, args.epochs):
        print("----> starting train")
        train_epoch(model, criterion1, criterion2, optimizer, train_dl, epoch, device, summary)
        # scheduler.step()
