...
def get_args():
    ...
    # Architecture
    parser.add_argument('--arch', type=str, default='mynet_ablation',
                        choices=['mynet', 'unet', 'attunet', 'unet++', 'deeplabv3+'],
                        help='Network architecture. unet or resnet')
    parser.add_argument('--ablation', type=str, default='acm + max + spe',
                        choices=['activeLoss', 'bd_loss', 'BCEDLoss'],
                        help='Network architecture.')

    # ACM
    parser.add_argument('--acm_iter', type=int, default=3, help='ACM iter.')

    # Data
    parser.add_argument('--train-dataset', type=str, default='dataset37', help='Training dataset')
    parser.add_argument('--data-path', type=str, default='/root/projects/Datasets/dataset37',
...
def train_epoch(model, criterion1, criterion2, optimizer, data_loader, epoch, device, summary):
    ...
    for idx, image_and_mask in enumerate(iterator):
        ...
        mask_pred = ACM(mask_pred, image, mask, args.acm_iter)
...
def val_epoch(model, data_loader, epoch, device, summary):
    ...
    for idx, image_and_mask in enumerate(data_loader):
        ...
        mask_pred = model(image)
        mask_pred = ACM(mask_pred, image, mask_gt, args.acm_iter)
