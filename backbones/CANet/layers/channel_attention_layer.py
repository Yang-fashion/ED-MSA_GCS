...
class SE_Conv_Block(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, drop_out=False):
        ...
        size = [240, 120, 60, 30, 15]
        # size = [256, 128, 64, 32, 16]
        # size = [288, 144, 72, 36, 18]

        if planes <= 32:
            self.globalAvgPool = nn.AvgPool2d(size[0], stride=1)  # (224, 300) for ISIC2018
            self.globalMaxPool = nn.MaxPool2d(size[0], stride=1)
        elif planes == 64:
            self.globalAvgPool = nn.AvgPool2d(size[1], stride=1)  # (112, 150) for ISIC2018
            self.globalMaxPool = nn.MaxPool2d(size[1], stride=1)
        elif planes == 128:
            self.globalAvgPool = nn.AvgPool2d(size[2], stride=1)    # (56, 75) for ISIC2018
            self.globalMaxPool = nn.MaxPool2d(size[2], stride=1)
        elif planes == 256:
            self.globalAvgPool = nn.AvgPool2d(size[3], stride=1)    # (28, 37) for ISIC2018
            self.globalMaxPool = nn.MaxPool2d(size[3], stride=1)
        elif planes == 512:
            self.globalAvgPool = nn.AvgPool2d(size[4], stride=1)    # (14, 18) for ISIC2018
            self.globalMaxPool = nn.MaxPool2d(size[4], stride=1)
